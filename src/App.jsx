...
function App() {
  ...
  useEffect(() => {
    // get info of city from gps device
    (async () => {
  ...
  useEffect(() => {
    // actualiza cada vez que app.city se actualize en el state
    (async () => {
  ...
  useEffect(() => {
    // actualiza cada vez q las coordenadas se actualizan, y pone pantalla en dia o noche
    (async () => {
  ...
  useEffect(() => {
    // si el fondo es oscuro pone el buscador claro, sino al reves 
    if (app.isDark) {
      document
        .querySelector(":root")
